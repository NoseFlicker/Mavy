...
{
    internal static class DnsMessageCoder
    {
        #region Encode
        ...
        public static byte[] EncodeDnsMessage(DnsMessage message)
        {
            ...
            var header = EncodeHeader(message);
            writer.Write(header);

            foreach (var query in message.Querys) writer.Write(EncodeDnsQuery(query));
            foreach (var answer in message.Answers) writer.Write(EncodeDnsResource(answer));
            foreach (var record in message.AuthorityRecords) writer.Write(EncodeDnsResource(record));
            foreach (var record in message.AdditionalRecords) writer.Write(EncodeDnsResource(record));
         ...
         * Header Structure, the bit labeled 0 is the most significant bit.
         ...
         +--+--+--+--+--+--+--+--+--+--+--+--+--+--+--+--+
        */
        private static byte[] EncodeHeader(DnsMessage message)
        {
            ...
            var fields = (int)message.ResponseCode;
            if (message.IsRecursionAvailable) fields |= 0x80;
            if (message.IsRecursionDesired) fields |= 0x100;
            if (message.IsTruncated) fields |= 0x200;
            if (message.IsRecursionAvailable) fields |= 0x400;
            fields |= (int)message.QueryKind << 11; //Opcode
            if (message.Type == MessageType.Response) fields |= 0x8000;
        ...
        QNAME
         ...
         padding is used.
        */
        private static byte[] EncodeDnsQuery(DnsQuery query)
        {
            var stream = new MemoryStream(256);
            var writer = new BinaryWriter(stream);

            //QNAME
            var labels = query.DomainName.Split('.');
            ...
            writer.Write((byte)0);
            //QTYPE
            WriteUInt16BE(writer, (ushort)query.QueryType);
            //QCLASS
            WriteUInt16BE(writer, (ushort)query.QueryClass);
            return stream.ToArray();
        }
